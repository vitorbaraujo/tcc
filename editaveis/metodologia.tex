...
\section{Metodologia de desenvolvimento}
  ...
  \subsection{Desenvolvimento da \textit{Engine}}
    ...
    Onde os métodos \texttt{update()} e \texttt{draw()}, abstratos, tratam de atualizar o objeto a cada frame e renderizar o objeto na posição \texttt{(x, y)}, respectivamente.

    É importante frisar que os métodos \texttt{update()} e \texttt{draw()} são abstratos e, considerando que o jogo será portado utilizando C++, estes métodos devem ser puramente virtuais, pois isso garante que qualquer classe que venha a extender de \texttt{GameObject} seja obrigada a implementer suas próprias rotinas específicas de atualização e renderização.

    Além da classe \texttt{GameObject}, os principais componentes da \textit{engine} a serem implementados são: módulo de vídeo, módulo de áudio, módulo de física e módulo de \textit{input}.
