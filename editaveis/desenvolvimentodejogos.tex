...
\begin{itemize}
\item \textbf{Módulo de vídeo:} módulo responsável por realizar a renderização de imagens, animações e textos. O vídeo de um jogo é composto por toda a parte de gameplay (onde o usuário está efetivamente jogando o jogo) e a parte de \textit{front-end}, que, segundo \citeonline[p.~39]{gregory2014game}, é composta pelo \textit{heads-up display} (HUD), menus e qualquer interface de usuário presente no jogo.
...
Além dos componentes gerais, todo jogo contém um mecanismo que é responsável pelo controle do estado atual do jogo, conhecido como \textit{game loop}. \citeonline{gameplayunb} caracterizam a estrutura de um \textit{game loop} por quatro tipos de tarefas (utilizando como exemplo uma partida do jogo Tetris):
